...
def limit_functions(p: RfunProgramT) -> RfunProgramT:
    ...
    def limit_functions_def(defn: RfunDefT, env: Dict[str, RfunExpT]) -> RfunDefT:
        if len(defn.args) > 6:
            # Replace the 6th argument with a single vector to hold the rest of the args
            # Generate a unique name for this 6th arg
            # Replace references to the 6+ arguments with vectorRef expressions that return the correct thing
            args_vec_name = gensym('args_vec')
            first_five_args = defn.args[:5]
            overflow_args = defn.args[5:]
            test('first_five_args', first_five_args)
            test('overflow_args', overflow_args)


            # This env maps variable names to vectorRef expressions
            # The variables in the env are the arg names for arg #6 and beyond
            # env: Dict[str, RfunExpT] = {}
            overflow_arg_exps = []
            overflow_arg_typs = []
            for i, a in enumerate(overflow_args):
                a_name, a_typ = a
                overflow_arg_exps.append(VarTE(args_vec_name, a_typ))
                overflow_arg_typs.append(a_typ)
                env[a_name] = PrimTE('vectorRef', [VarTE(a_name, a_typ), IntTE(i)], a_typ)

            test('env', env)

            overflow_args_vec = PrimTE('vector', overflow_arg_exps, VectorT(overflow_arg_typs))
            new_body = limit_functions_exp(defn.body, env)
            new_args_list_total = first_five_args + [overflow_args_vec]
            temp = RfunDefT(defn.name, new_args_list_total, defn.output_type, new_body)
            test('return vector', temp)

            return RfunDefT(defn.name, first_five_args + [overflow_args_vec], defn.output_type, new_body)
        else:
    ...
    def limit_functions_exp(e: RfunExpT, env: Dict[str, RfunExpT]) -> RfunExpT:
        if isinstance(e, (IntTE, BoolTE)):
            ...
            # In a function call with more than 6 arguments, replace the 6+ arguments
            # with a single call to the vector primitive containing argument 6 and beyond
            if len(e.args) > 6:
                first_five_args = new_args[:5]
                overflow_args = new_args[5:]
                if isinstance(e.fun, VarTE):
                    test('testing...')
                    # TODO not sure if this is right
                    assert isinstance(e.typ, VectorT)
                    vec_typ = e.typ.types
                elif isinstance(e.fun, FunRefTE):
                    vec_typ = e.fun.typ.arg_types
                else:
                    raise Exception('limit_functions_exp FuncallTE', e)
                vec_expr = PrimTE('vector', overflow_args, VectorT(vec_typ))

                return FuncallTE(new_fun, first_five_args + [vec_expr], e.typ)
            else:
    ...
    # TODO This might be wrong, set may need to change to dict
    # This should be a set of the names of top-level definitions
    env_top_level = set(d.name for d in p.defs)
    env_top_level = {}
    test('env_top_level', env_top_level)
    test('p.defs', p.defs)

    new_defs = [limit_functions_def(d, env_top_level) for d in p.defs]
    new_body = limit_functions_exp(p.body, env_top_level)
